...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Usuarios
      let url = `http://192.168.1.10:3000/api/admin/users?`;
      ...
      // Transacciones
      const resTx = await fetch('http://192.168.1.10:3000/api/admin/transactions', { headers: { Authorization: `Bearer ${token}` } });
      const dataTx = await resTx.json();
      setTransactions(dataTx.transactions || []);

      // Solicitudes
      const resReq = await fetch('http://192.168.1.10:3000/api/admin/requests', { headers: { Authorization: `Bearer ${token}` } });
  ...
  const eliminarUsuario = async (id: number) => {
    try {
      const token = await AsyncStorage.getItem('token');
      const res = await fetch(`http://192.168.1.10:3000/api/admin/users/${id}`, {
        method: 'DELETE',
  ...
  const bloquearUsuario = async (id: number) => {
    try {
      const token = await AsyncStorage.getItem('token');
      const res = await fetch(`http://192.168.1.10:3000/api/admin/users/${id}/block`, {
        method: 'PUT',
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
  ...
  const desbloquearUsuario = async (id: number) => {
    try {
      const token = await AsyncStorage.getItem('token');
      const res = await fetch(`http://192.168.1.10:3000/api/admin/users/${id}/unblock`, {
        method: 'PUT',
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
  ...
  const guardarCambiosUsuario = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://192.168.1.10:3000/api/admin/users/${editUser.id}`, {
        method: 'PUT',
        headers: {
