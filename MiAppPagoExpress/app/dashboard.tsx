...
export default function DashboardScreen() {
  ...
  useEffect(() => {
    const checkRole = async () => {
      ...
      const res = await fetch('http://192.168.1.10:3000/api/user/profile', {
  ...
  const fetchProfileAndBalance = async () => {
    ...
    try {
      // Obtener perfil
      const resProfile = await fetch('http://192.168.1.10:3000/api/user/profile', {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (resProfile.ok) {
      ...
      // Obtener saldo
      const resBalance = await fetch('http://192.168.1.10:3000/api/user/balance', {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleRecharge = async () => {
    ...
    try {
      const token = await AsyncStorage.getItem('token');
      const res = await fetch('http://192.168.1.10:3000/api/user/recharge', {
        method: 'POST',
        headers: {
