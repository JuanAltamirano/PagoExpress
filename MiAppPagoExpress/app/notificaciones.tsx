...
export default function NotificacionesScreen() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Obtener solicitudes
      const resReq = await fetch('http://192.168.1.10:3000/api/payment/requests', {
      ...
      // Obtener saldo
      const resBal = await fetch('http://192.168.1.10:3000/api/user/balance', {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const aceptarSolicitud = async (id: number, amount: number) => {
    ...
    try {
      const token = await AsyncStorage.getItem('token');
      const response = await fetch(`http://192.168.1.10:3000/api/payment/requests/${id}/accept`, {
        method: 'PUT',
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const denegarSolicitud = async (id: number) => {
    try {
      const token = await AsyncStorage.getItem('token');
      const response = await fetch(`http://192.168.1.10:3000/api/payment/requests/${id}/reject`, {
        method: 'PUT',
        headers: { Authorization: `Bearer ${token}` },
      });
